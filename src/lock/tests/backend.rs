...
//! (`crate::lock::backend`): owner-gated acquire / refresh / release.

use redis::aio::ConnectionManager;

use crate::lock::backend;
...
async fn acquire_is_exclusive() {
    ...
    assert!(
        backend::acquire(&mut conn, &key, OWNER_A, 30_000)
            .await
            .unwrap(),
        "first acquire should take the lock"
    );
    assert!(
        !backend::acquire(&mut conn, &key, OWNER_B, 30_000)
            .await
            .unwrap(),
...
async fn refresh_is_owner_gated() {
    ...
    assert!(
        backend::acquire(&mut conn, &key, OWNER_A, 2_000)
            .await
            .unwrap()
    );

    assert!(
        backend::refresh(&mut conn, &key, OWNER_A, 30_000)
            .await
            .unwrap(),
    ...
    assert!(
        !backend::refresh(&mut conn, &key, OWNER_B, 30_000)
            .await
            .unwrap(),
...
async fn release_is_owner_gated() {
    ...
    assert!(
        !backend::release(&mut conn, &key, OWNER_B).await.unwrap(),
    ...
    assert!(
        backend::release(&mut conn, &key, OWNER_A).await.unwrap(),
...
async fn acquire_succeeds_after_release() {
    ...
    assert!(backend::release(&mut conn, &key, OWNER_A).await.unwrap());
    assert!(
        backend::acquire(&mut conn, &key, OWNER_B, 30_000)
            .await
            .unwrap(),
...
async fn lease_expiry_frees_key() {
    ...
    assert!(backend::acquire(&mut conn, &key, OWNER_A, 100).await.unwrap());
